...
//   // First register each node’s declared aliases
//   for (const node of nodes) {
//     if (node.aliases) {
//       for (const alias of node.aliases) {
//         const key = alias.toLowerCase();
//         if (!map.has(key)) map.set(key, node); // keep first owner
//       }
//     }
//   }

//   // Optional second pass: register [[links]] that don’t already exist
//   for (const node of nodes) {
//     const regex = /\[\[(.*?)\]\]/g;
//     let match: RegExpExecArray | null;
//     while ((match = regex.exec(node.text)) !== null) {
